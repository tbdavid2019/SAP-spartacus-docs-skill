...
Locked elements leverage the auto focus feature, unless the auto focus configuration is explicitly set to false. This results in the first focusable element being selected when the group is unlocked. For more information, see [Auto Focus](auto-focus.md).

An unlocked group can be locked again by pressing the `ESC` key. The locked focus feature uses the escape focus feature, so that the host element is focused again when pressing `ESC`. For more information, see [Escape Focus](escape-focus.md).
